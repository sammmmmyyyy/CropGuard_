...
async function findStorage() {
    ...
    try {
        ...
        querySnapshot.forEach((docSnapshot) => {
            let vendor = docSnapshot.data();
            vendor.id = docSnapshot.id; // Store document ID
        ...
        if (nearestVendor) {
            ...
            await updateAvailableSpace(nearestVendor.id, nearestVendor.availableSpace, quantity);
        } else {
...
// Function to update available storage space
async function updateAvailableSpace(vendorId, currentSpace, quantity) {
    if (currentSpace >= quantity) {
        const vendorRef = doc(db, "vendors", vendorId);
        await updateDoc(vendorRef, {
            availableSpace: currentSpace - quantity
        });
        alert("Storage space updated successfully!");
    } else {
        alert("Not enough available space in selected storage.");
    }
}
